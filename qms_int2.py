...
def read_gnu_debuglink(real_elf: str, readelf_bin: str) -> Optional[str]:
    """
    Try to read .gnu_debuglink via '<readelf_bin> --string-dump=.gnu_debuglink'.
    Returns the debug file name or None.
    """
    try:
        out = subprocess.check_output(
            [readelf_bin, "--string-dump=.gnu_debuglink", real_elf],
            stderr=subprocess.DEVNULL,
            text=True,
        )
    except Exception:
...
def resolve_target_elf_gnu(
    ...
    # 1) gnu_debuglink
    dbg_link = read_gnu_debuglink(real_orig, readelf_bin)
    if dbg_link:
...
def build_jobs_by_target(
    ...
    for key, info in origins.items():
        ...
        if readelf_bin is None:
            readelf_bin_local = "readelf"
        else:
            readelf_bin_local = readelf_bin
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Cross toolchain prefix (e.g. aarch64-linux-gnu-). Will prepend this "
    ...
    args.workers_symbols = _normalize_workers("workers-symbols", args.workers_symbols)
    args.workers_rewrite = _normalize_workers("workers-rewrite", args.workers_rewrite)

    # Determine actual worker counts (backup-style semantics)
    # Symbol workers:
    #   0 => auto (based on CPU and available memory)
    #   1 => no parallelism (single process)
    #   N>1 => use N workers
    if args.workers_symbols == 0:
        available = get_available_memory_bytes()
        if available is not None:
            # assume ~300MB per worker as a rough bound
            mem_workers = max(1, available // (300 * 1024 * 1024))
            workers_symbols = max(1, min(cpu_count(), mem_workers))
        else:
            workers_symbols = cpu_count()
    else:
        workers_symbols = args.workers_symbols

    # Rewrite workers:
    #   0 => auto (max(4, CPU count))
    #   1 => no parallelism (single thread)
    #   N>1 => use N workers
    ...
    # Overwrite args with final integer values so the rest of the code can use them directly.
    args.workers_symbols = int(workers_symbols)
    args.workers_rewrite = int(workers_rewrite)
    ...
    # Decide which readelf binary to use (backup-style):
    #   - In GNU mode: cross_prefix + "readelf" if cross_prefix is set
    #   - Otherwise: plain "readelf"
    if args.mode == "gnu" and args.cross_prefix:
        readelf_bin = args.cross_prefix + "readelf"
    else:
        readelf_bin = "readelf"
    ...
    jobs_by_target, target_for_origin, debug_failures = build_jobs_by_target(
        ...
        readelf_bin=readelf_bin if args.mode == "gnu" else None,
    )
    if benchmark:
        t_now = perf_counter()
